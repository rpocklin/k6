...
func BenchmarkParseTrend(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseTrend()([]rune("p(99.99"))
	}
}
...
func TestParsePercentile_OutOfBoundsValue(t *testing.T) {
	t.Parallel()
	...
	// Act
	underflowResult := parser([]rune("p(-1)"))
	overflowResult := parser([]rune("p(100.1)"))

	// Assert
	assert.NotNil(t, overflowResult.Err, "parsing percentile < 0 should fail")
	assert.Equal(t, "p(100.1)", string(overflowResult.Err.Input), "overflowing percentile parsing should leave input untouched")
	assert.NotNil(t, underflowResult.Err, "parsing percentile > 100 should fail")
	assert.Equal(t, "p(-1)", string(underflowResult.Err.Input), "underflowing percentile parsing should leave input untouched")
}

func BenchmarkParsePercentile(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParsePercentile()([]rune("p(99.99)"))
	}
}
