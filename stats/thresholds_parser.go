...
func ParsePercentile() combinators.Parser {
	...
	return func(input []rune) combinators.Result {
		...
		value, _ := parsed[1].(float64) // If the Float combinator succeeded, parsed[1] will be a float64
		if value < 0 || value > 100 {
			err := fmt.Errorf("invalid percentile value %g, must be within [0, 100] bounds", value)
			res.Err = combinators.NewFatalError(input, err, "percentile")
			return res
		}
